 ///
 /// @file    log4cpp_config.cc
 /// @author  WildFiretobee(Danderous@email.com)
 /// @date    2018-04-15 10:13:12
 ///
...
int main(int argc,char* argv[])
{
	...
	//BasicLayout 输出所有优先级日志到ConsoleAppender
	try
	{
	...
	// sub2 has appender A2 and rootappender. 
	//since the additivity property is set to false
	
	sub2.alert("sub2's log");

	// sub3 has appender A3 and rootappender.
	// since the additivity property is set to false 
	sub3.debug("sub3's log");
	sub3.alert("sub3's log");

	// sub4 can not be found in the config file,so the root category
	// 's appender and layout are used
